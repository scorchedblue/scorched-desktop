...
- **Only bind programs the image ships.** A binding to something absent fails
  silently. `foot`, `fuzzel`, `swaylock`, `grim`, `slurp`, `mako` are present;
  `kitty`, `dolphin` and `hyprlauncher` are not, which is what makes Hyprland's
  autogenerated default useless here.
...
- **`hyprctl dispatch` takes Lua too.** `hyprctl dispatch exec foot` fails in
  0.56; it is `hyprctl dispatch 'hl.dsp.exec_cmd("foot")'`. The old string
